...
# 使用项目前必看<项目视频百度云链接.txt>的第一行有一个必看的视频!!
   ...
   # model = YOLO('ultralytics-main/ultralytics-main/ultralytics/cfg/models/11/yolo11-ECA.yaml')
    # model.load('yolo11n.pt') # loading pretrain weights
# ========== 将 ECA 注入到 ultralytics 模块中 ==========
# 方法1：直接挂载到 ultralytics.nn.modules（让 globals() 能找到）
#import ultralytics.nn.modules
#ultralytics.nn.modules.MSHC = MSHC

# 方法2：修改 tasks 模块的 globals（这是 parse_model 查找的地方）
#import ultralytics.nn.tasks
#ultralytics.nn.tasks.MSHC = MSHC

# 如果上面两种还不够，还可以强制替换 parse_model 函数（最暴力）
# 但通常上面两步就足够了
...
if __name__ == '__main__':
    ...
    model.train(data='dataset/2nd-guanjiang.v1i.yolov11/data.yaml',
                cache=False,
                imgsz=640,
                epochs=50,
                batch=32,
